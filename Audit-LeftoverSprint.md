...
- [ ] 🟠 **F2 (High, = Part 2 commission-ledger gap) — commission computed and discarded, AND the whole pricing/commission model was wrong.** Confirmed originally: `SupplierPayable` had no `commissionAmount` column. **In the course of fixing this (2026-07-24) the product owner clarified the real model, which the code did not implement at all:**
  ...
  **Remaining:**
  - **Part C (consumables)** — a 7% payout ledger for Buy Now + bulk orders (generalize `SupplierPayable` beyond bookings, or a sibling table), snapshotting the effective consumables commission. Consumables *charging* is already correct (member pays RSP, no markup); this is only the missing supplier-payout/commission side.
  - **Part D (reconciliation)** — the three-way breakdown (grossCollected = commissionKept + supplierNet, where commissionKept = `commissionAmount` + `penaltyDeduction`) per pending-company, per payout batch, and platform-wide, surfaced in the admin panel. Also resolve **F5** here: the supplier's own "Platform Revenue" chart (`getCompanyRevenueByTypeAndMonth`) now sums the **marked-up member price**, which overstates what the supplier actually earns (they get 90% of base) — decide whether that chart should show gross marketplace volume or the supplier's net payout.
